...
class SistemaAcionistas {
    public List<String> obterAnalisesDesempenho(String dataInicialStr, String dataFinalStr) throws ParseException {
        ...
        // filtrar as análises.
        List<Analise> analisesFiltradas = analises.stream()
            .filter(analise -> (analise.data.equals(dataInicial) || analise.data.after(dataInicial))
                    && (analise.data.equals(dataFinal) || analise.data.before(dataFinal)))
            .collect(Collectors.toList());

        // TODO: Retorne a lista de análises filtradas.
        List<String> analisesFormatadas = analisesFiltradas.stream()
        .map(analise -> analise.descricao)
        .collect(Collectors.toList());

        return analisesFormatadas;
    }
